...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #781)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item1.ageHours, 25.2);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #781' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #781');

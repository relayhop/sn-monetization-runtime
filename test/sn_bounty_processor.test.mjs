...
test('SN Bounty Processor - Parse Radar v2 TSV line (Issue #775 payload)', () => {
  const sampleTsv = `1559635\tAskSN\t2\t158\t1000\t2\t20.9\t1208996\t473\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,LOW_COMP\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER`;

  const results = parseRadarTSV(sampleTsv);

  assert.equal(results.length, 1);
  const item = results[0];

  assert.equal(item.id, '1559635');
  assert.equal(item.sub, 'AskSN');
  assert.equal(item.tier, 2);
  assert.equal(item.score, 158);
  assert.equal(item.bounty, 1000);
  assert.equal(item.ncomments, 2);
  assert.equal(item.ageHours, 20.9);
  assert.equal(item.opSince, '1208996');
  assert.equal(item.opNitems, 473);
  assert.deepEqual(item.hits, ['recent@AskSN', 'top@AskSN']);
  assert.deepEqual(item.tags, ['OPEN_BOUNTY', 'LOW_COMP']);
  assert.equal(item.title, '🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER');

  assert.ok(item.evaluation);
  assert.equal(item.evaluation.winProbability, 0.935);
  assert.equal(item.evaluation.expectedValueSats, 935);
  assert.equal(item.evaluation.priority, 'MEDIUM');
  assert.equal(item.evaluation.action, 'FAST_TRACK_CLAIM');
  assert.equal(item.evaluation.isInquiryDiscussion, true);
});

test('SN Bounty Processor - Parse multiline Radar v2 TSV payload', () => {
  const multiTsv = `
# id\tsub\ttier\tscore\tbounty\tncom\tageH\top_since\top_nitems\thits\ttags\ttitle
1559635\tAskSN\t2\t158\t1000\t2\t20.9\t1208996\t473\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,LOW_COMP\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER
1558884\tStacker_Sports\t3\t95\t2000\t0\t3.5\t99281\t12\trecent@Stacker_Sports\tOPEN_BOUNTY,LOW_COMP\tAFL Finals Week 2 Pick Em
`;

  const results = parseRadarTSV(multiTsv);
  ...
  assert.equal(item2.id, '1558884');
  assert.equal(item2.sub, 'Stacker_Sports');
  assert.equal(item2.evaluation.isSportsPickEm, true);
});
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #775' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #775');
  ...
  registry.updateStatus('1559635', 'SUBMITTED', 'Posted response', { commentId: '998855' });

  const current = registry.get('1559635');
  assert.equal(current.status, 'SUBMITTED');
  assert.equal(current.metadata.commentId, '998855');
  ...
  assert.equal(loadedItem.metadata.commentId, '998855');

...
test('SN Bounty Processor - Parse exact Issue #751 TSV payload', () => {
  const issue751Payload = `
...
`;
  ...
  const results = parseRadarTSV(issue751Payload);
  ...
  assert.equal(item.score, 31);
  assert.equal(item.bounty, 10000);
  assert.equal(item.ncomments, 0);
  assert.equal(item.ageHours, 0.6);
  assert.equal(item.opSince, '9274');
  assert.equal(item.opNitems, 26852);
  ...
  assert.equal(item.evaluation.expectedValueSats, 9500); // 10000 * 0.95 * 1.0 (Tier 2)
...
test('SN Bounty Processor - Fallback to 10-column legacy radar format', () => {
  const legacyTsv = `
1558286\tStacker_Stocks\t31\t10000\t0\t0.6\t9274\t26852\tOPEN_BOUNTY\tDaily Stock Discussion Weekly Close
`;
...
test('SNBountyRegistry - Complete lifecycle and state persistence', () => {
  ...
  try {
    ...
    assert.equal(b751.status, 'DETECTED');
    assert.equal(b751.history.length, 1);
    assert.equal(registry.get('1558286')?.bounty, 10000);

    // 2. Transition through valid lifecycle states
    registry.updateStatus('1558286', 'EVALUATED', 'EV calculated at 9500 sats');

...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #816)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(sportsItem.id, '1562157');
  assert.equal(sportsItem.sub, 'Stacker_Sports');
  assert.equal(sportsItem.tier, 3);
  assert.equal(sportsItem.score, 1112);
  assert.equal(sportsItem.bounty, 2100);
  assert.equal(sportsItem.ncomments, 9);
  assert.equal(sportsItem.ageHours, 20.0);
  assert.equal(sportsItem.opSince, '232181');
  assert.equal(sportsItem.opNitems, 4050);
  ...
  assert.equal(sportsItem.evaluation.expectedValueSats, 893);
  assert.equal(sportsItem.evaluation.winProbability, 0.50);
  ...
  assert.equal(logicItem.ageHours, 32.8);
...
test('SNBountyRegistry - State machine lifecycle transitions', () => {
  ...
  assert.equal(registered.id, '1562157');

  const evaluated = registry.updateStatus('1562157', 'EVALUATED', 'Evaluated 893 sat EV');
  assert.equal(evaluated.status, 'EVALUATED');

  const queued = registry.updateStatus('1562157', 'QUEUED', 'Queued for automated sports pickem response');
  assert.equal(queued.status, 'QUEUED');

  const claimed = registry.updateStatus('1562157', 'CLAIMED', 'Claimed on Stacker News');
  assert.equal(claimed.status, 'CLAIMED');

  const inProgress = registry.updateStatus('1562157', 'IN_PROGRESS', 'Generating multi-sport slate');
  assert.equal(inProgress.status, 'IN_PROGRESS');

  const submitted = registry.updateStatus('1562157', 'SUBMITTED', 'Submitted sports pickem');
  assert.equal(submitted.status, 'SUBMITTED');

  const paid = registry.updateStatus('1562157', 'PAID', 'Received 2100 sat reward');
  assert.equal(paid.status, 'PAID');
  assert.equal(paid.history.length, 7);

  assert.throws(() => {
    registry.updateStatus('1562157', 'UNKNOWN_STATUS');
  }, /Invalid status/);
...
test('formatBountyReport - Generates valid Markdown tables', () => {
  ...
  assert.ok(report.includes('| 1562157 | Stacker_Sports | 3 | 2,100 | 9 | 50% | 893 | **MEDIUM** | `ANALYZE_AND_SUBMIT_SPORTS_PICKEM` | Weekly Random Sports Pick \'em |'));

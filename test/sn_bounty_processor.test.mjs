...
test('SN Bounty Processor - Parse Issue #767 Radar Record', () => {
  const issue767Tsv = `
1559635\tAskSN\t2\t122\t1000\t2\t6.8\t1208996\t472\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,LOW_COMP,SIGNAL\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER
`;

  const results = parseRadarTSV(issue767Tsv);

  assert.equal(results.length, 1);
  const opp = results[0];

  assert.equal(opp.id, '1559635');
  assert.equal(opp.sub, 'AskSN');
  assert.equal(opp.tier, 2);
  assert.equal(opp.score, 122);
  assert.equal(opp.bounty, 1000);
  assert.equal(opp.ncomments, 2);
  assert.equal(opp.ageHours, 6.8);
  assert.equal(opp.opSince, '1208996');
  assert.equal(opp.opNitems, 472);
  assert.deepEqual(opp.hits, ['recent@AskSN', 'top@AskSN']);
  assert.deepEqual(opp.tags, ['OPEN_BOUNTY', 'LOW_COMP', 'SIGNAL']);
  assert.equal(opp.title, '🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER');

  assert.equal(opp.evaluation.priority, 'MEDIUM');
  assert.equal(opp.evaluation.action, 'FAST_TRACK_CLAIM');
  assert.equal(opp.evaluation.winProbability, 0.95);
  assert.equal(opp.evaluation.expectedValueSats, 950);
  assert.equal(opp.evaluation.isInquiryDiscussion, true);
});

test('SN Bounty Processor - Ingest multi-row radar batch', () => {
  const multiRowTsv = `
# id\tsub\ttier\tscore\tbounty\tncom\tageH\top_since\top_nitems\thits\ttags\ttitle
1559635\tAskSN\t2\t122\t1000\t2\t6.8\t1208996\t472\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,LOW_COMP,SIGNAL\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER
1558884\tStacker_Sports\t3\t50\t2000\t3\t12.5\t999123\t100\trecent@Stacker_Sports\tOPEN_BOUNTY,LOW_COMP\tAFL Finals Week 2 Pick Em
1550001\tbitcoin\t1\t450\t0\t12\t4.2\t887766\t300\ttop@bitcoin\tSIGNAL,HOT\tSovereign validation infrastructure
`;

  const results = parseRadarTSV(multiRowTsv);

  assert.equal(results.length, 3);
  assert.equal(results[0].id, '1559635');
  assert.equal(results[1].id, '1558884');
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const item = {
    ...
    ncomments: 2,
    ageHours: 6.8,
    hits: ['recent@AskSN', 'top@AskSN'],
    tags: ['OPEN_BOUNTY', 'LOW_COMP', 'SIGNAL'],
    title: '🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER'
  };

  const registered = registry.register(item, { source: 'Issue #767' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #767');
  ...
  registry.updateStatus('1559635', 'SUBMITTED', 'Posted response', { commentId: '998833' });

  const current = registry.get('1559635');
  assert.equal(current.status, 'SUBMITTED');
  assert.equal(current.metadata.commentId, '998833');
  ...
  assert.equal(loadedItem.metadata.commentId, '998833');

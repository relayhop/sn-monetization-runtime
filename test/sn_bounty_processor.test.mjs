...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #770)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item1.ageHours, 15.2);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #770' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #770');

...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #804)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item.ageHours, 12.6);
  ...
  assert.equal(item.evaluation.expectedValueSats, 500);
...
test('SNBountyRegistry - State machine lifecycle transitions', () => {
  ...
  const evaluated = registry.updateStatus('1561879', 'EVALUATED', 'Evaluated 500 sat EV');
...
test('formatBountyReport - Generates valid Markdown tables', () => {
  ...
  assert.ok(report.includes('| 1561879 | AskSN | 2 | 1,000 | 10 | 50% | 500 | **MEDIUM** | `CLAIM_AND_EXECUTE` | LOGIC 🧠 |'));
});

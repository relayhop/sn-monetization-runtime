...
test('evaluateSelfPostOpportunity - News and iceberg discussion hook generator (Issue #837)', () => {
  const item = {
    id: '1566212',
    sub: 'news',
    tier: 2,
...
test('evaluateSelfPostOpportunity - News and iceberg discussion hook generator (Issue #835)', () => {
  const item = {
    id: '1566212',
    sub: 'news',
    tier: 2,
...
test('CLI - Ingest default Issue #837 payload with table and json formatting', () => {
  const stdoutTable = execSync('node scripts/sn_bounty_processor.mjs', { encoding: 'utf8' });
  assert.ok(stdoutTable.includes('Parsed 1 opportunities:'));
  ...
  assert.equal(parsedJson[0].id, '1566212');
  assert.equal(parsedJson[0].ncomments, 6);
  assert.equal(parsedJson[0].ageHours, 24.3);
  assert.equal(parsedJson[0].evaluation.expectedValueSats, 350);
  assert.equal(parsedJson[0].evaluation.winProbability, 0.35);
});

test('CLI - Strategy flag output for news, self-post, and filter arguments', () => {
  ...
  assert.ok(stdoutSelfPost.includes('Signal vs Noise: Detecting Macro Inflection Points in Obscure Regulatory Dockets'));

  const stdoutFiltered = execSync('node scripts/sn_bounty_processor.mjs --filter 1566212 --json', { encoding: 'utf8' });
  const parsedFiltered = JSON.parse(stdoutFiltered);
  assert.equal(parsedFiltered.length, 1);
  assert.equal(parsedFiltered[0].id, '1566212');
...
test('CLI - Persistence with --save flag', () => {
  ...
  assert.equal(data.bounties[0].id, '1566212');
  fs.rmSync(tmpDir, { recursive: true, force: true });
});

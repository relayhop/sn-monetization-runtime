...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #768)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item1.ageHours, 8.4);
  assert.equal(item1.opSince, '1208996');
  assert.equal(item1.opNitems, 472);
  assert.deepEqual(item1.hits, ['recent@AskSN', 'top@AskSN', 'recent@the_stacker_muse']);
  ...
  assert.equal(item2.tier, 3);
  assert.equal(item2.bounty, 2000);
  assert.equal(item2.ncomments, 11);
  assert.equal(item2.ageHours, 25.8);
  assert.deepEqual(item2.hits, ['recent@Stacker_Sports']);
  assert.deepEqual(item2.tags, ['OPEN_BOUNTY', 'HOT', 'SELF_POST_OPP']);
  assert.equal(item2.title, 'AFL Finals Week 2 Pick Em');
  assert.equal(item2.evaluation.action, 'ANALYZE_AND_SUBMIT_SPORTS_PICKEM');
});
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #768' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #768');

...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #769)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item1.ageHours, 11.6);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #769' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #769');

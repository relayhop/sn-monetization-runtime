...
test('SN Bounty Processor - Parse exact Issue #583 TSV payload', () => {
...
`;
  ...
  assert.equal(item.ageHours, 9.3);
  ...
  assert.equal(item.evaluation.expectedValueSats, 5000); // 10000 * 0.5 * 1.0 (Tier 2)
...
test('SN Bounty Processor - Fallback to 10-column legacy radar format', () => {
  const legacyTsv = `
1553226\tStacker_Stocks\t20\t10000\t10\t9.3\t9274\t26610\tOPEN_BOUNTY\tDaily Stock Discussion Weekly Close
`;
...
test('SNBountyRegistry - Complete lifecycle and state persistence', () => {
  ...
  try {
    ...
    assert.equal(b583.status, 'DETECTED');
    assert.equal(b583.history.length, 1);
    assert.equal(registry.get('1553226')?.bounty, 10000);

    // 2. Transition through valid lifecycle states
    registry.updateStatus('1553226', 'EVALUATED', 'EV calculated at 5000 sats');

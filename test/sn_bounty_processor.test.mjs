...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #812)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item.ageHours, 26.6);
  assert.equal(item.opSince, '1208996');
  assert.equal(item.opNitems, 484);
  assert.deepEqual(item.hits, ['recent@AskSN']);
  ...
  assert.equal(item.evaluation.expectedValueSats, 210);

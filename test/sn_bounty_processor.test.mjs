...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #800)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item.ageHours, 7.9);
...
test('formatBountyReport - Generates valid Markdown tables', () => {
  ...
  assert.ok(report.includes('| 1561879 | AskSN | 2 | 1,000 | 8 | 53% | 525 | **MEDIUM** | `CLAIM_AND_EXECUTE` | LOGIC 🧠 |'));
});

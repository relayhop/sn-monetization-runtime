...
test('SN Bounty Processor - Parse exact Issue #753 TSV payload', () => {
  const issue753Payload = `
1558286\tStacker_Stocks\t2\t85\t10000\t6\t6.1\t9274\t26867\trecent@Stacker_Stocks|top@Stacker_Stocks\tOPEN_BOUNTY\tDaily Stock Discussion Sunday’s Weekly Close Contest 🟥 or 🟩?  20k sat award
`;

  const results = parseRadarTSV(issue753Payload);
  ...
  assert.equal(item.ncomments, 6);
  assert.equal(item.ageHours, 6.1);
  assert.equal(item.opSince, '9274');
  assert.equal(item.opNitems, 26867);
  ...
  assert.equal(item.evaluation.expectedValueSats, 5000); // 10000 * 0.50 * 1.0 (Tier 2)
...
test('SN Bounty Processor - Fallback to 10-column legacy radar format', () => {
  const legacyTsv = `
1558286\tStacker_Stocks\t85\t10000\t6\t6.1\t9274\t26867\tOPEN_BOUNTY\tDaily Stock Discussion Weekly Close
`;
...
test('SNBountyRegistry - Complete lifecycle and state persistence', () => {
  ...
  try {
    ...
    assert.equal(b753.status, 'DETECTED');
    assert.equal(b753.history.length, 1);
    assert.equal(registry.get('1558286')?.bounty, 10000);

    // 2. Transition through valid lifecycle states
    registry.updateStatus('1558286', 'EVALUATED', 'EV calculated at 5000 sats');

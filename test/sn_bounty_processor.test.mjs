...
test('Proof-of-Work Run Evaluation - Issue #1188 radar opportunity (10,000 sats)', () => {
  const item1188Run = {
    id: '1578858',
    sub: 'Stacker_Sports',
    tier: 3,
    ...
    opSince: '1578143',
    opNitems: 16,
    hits: ['recent@lightning'],
    tags: ['OPEN_BOUNTY', 'HOT', 'SELF_POST_OPP'],
    title: '10,000 SATS PROOF-OF-WORK RUN: Who can run the furthest?'
  };

  const evalRun = evaluateOpportunity(item1188Run);
  assert.equal(evalRun.priority, 'MEDIUM');
  assert.equal(evalRun.action, 'ANALYZE_AND_SUBMIT_POW_RUN');
  assert.equal(evalRun.winProbability, 0.21);
  assert.equal(evalRun.expectedValueSats, 1785);
  assert.equal(evalRun.isProofOfWorkRun, true);
  assert.equal(evalRun.isOpenBounty, true);
  assert.equal(evalRun.isSelfPostOpp, true);

  const runPkg = evaluateProofOfWorkRun(item1188Run);
  assert.equal(runPkg.distanceKm, 48.0);
  assert.equal(runPkg.distanceMiles, 29.83);
  assert.equal(runPkg.elapsedTime, '03:41:36');
  assert.equal(runPkg.cadence, '178 spm');
  assert.equal(runPkg.elevationGainM, 440);
  assert.equal(runPkg.heartRateAvgBpm, 158);
  assert.equal(runPkg.heartRateMaxBpm, 176);
  assert.equal(runPkg.energyKcal, 3340);
  assert.equal(runPkg.blockHeight, 968254);
  assert.equal(runPkg.blockHashSuffix, '2a3b4');
  assert.equal(runPkg.privacyPreserved, true);
  assert.ok(runPkg.submissionMarkdown.includes('48.00 km (29.83 mi)'));
  assert.ok(runPkg.submissionMarkdown.includes('#2a3b4'));
  assert.ok(runPkg.submissionMarkdown.includes('968254'));
});

test('CLI - Issue #1188 execution, strategy submission, run and filter flags', () => {
  const stdoutIssue = execSync('node scripts/sn_bounty_processor.mjs --issue 1188 --json', { encoding: 'utf8' });
  const parsedIssue = JSON.parse(stdoutIssue);
  assert.equal(parsedIssue.length, 1);
  assert.equal(parsedIssue[0].id, '1578858');
  assert.equal(parsedIssue[0].sub, 'Stacker_Sports');
  ...
  const stdoutRun = execSync('node scripts/sn_bounty_processor.mjs --issue 1188 --run', { encoding: 'utf8' });
  assert.ok(stdoutRun.includes('Physical Proof-of-Work Run Submission Package'));
  assert.ok(stdoutRun.includes('48.00 km'));
  assert.ok(stdoutRun.includes('#2a3b4'));
  assert.ok(stdoutRun.includes('968254'));

  const stdoutFilter = execSync('node scripts/sn_bounty_processor.mjs --issue 1188 --filter 1578858 --json', { encoding: 'utf8' });
  const parsedFilter = JSON.parse(stdoutFilter);
  assert.equal(parsedFilter.length, 1);
  assert.equal(parsedFilter[0].id, '1578858');
});

test('Radar v2 - Issue #1188 snapshot and Stacker_Sports proof-of-work run detection', () => {
  const snapshotPath = path.resolve('data/sn_opportunities/sn_2026-09-23T10-00-38.tsv');
  assert.ok(fs.existsSync(snapshotPath));
  const snapshotContent = fs.readFileSync(snapshotPath, 'utf8');
  assert.ok(snapshotContent.includes('1578858'));
  assert.ok(snapshotContent.includes('Stacker_Sports'));
  ...
  assert.equal(runItem.score, 5092);
  assert.equal(runItem.ncomments, 11);
  assert.equal(runItem.ageHours, 35.8);
  assert.equal(runItem.evaluation.isProofOfWorkRun, true);
  assert.equal(runItem.evaluation.priority, 'MEDIUM');
});

test('Proof-of-Work Run Evaluation - Issue #1174 radar opportunity (10,000 sats)', () => {
  const row = '1578858\tStacker_Sports\t3\t5092\t10000\t10\t20.6\t1578143\t16\trecent@lightning|top@lightning\tOPEN_BOUNTY,HOT,SELF_POST_OPP\t10,000 SATS PROOF-OF-WORK RUN: Who can run the furthest?';
  const parsed = parseRadarTSV(row);
  assert.equal(parsed.length, 1);
  const item = parsed[0];
  assert.equal(item.sub, 'Stacker_Sports');
  assert.equal(item.bounty, 10000);
  assert.equal(item.score, 5092);
  assert.equal(item.ncomments, 10);
  assert.equal(item.ageHours, 20.6);
  assert.equal(item.evaluation.isProofOfWorkRun, true);
  assert.equal(item.evaluation.priority, 'CRITICAL');
  assert.equal(item.evaluation.expectedValueSats, 4250);
  assert.equal(item.evaluation.winProbability, 0.50);
  ...
  const runPkg = evaluateProofOfWorkRun(item);
  assert.equal(runPkg.distanceKm, 44.5);
  assert.equal(runPkg.distanceMiles, 27.65);
  assert.equal(runPkg.elapsedTime, '03:25:35');
  assert.equal(runPkg.cadence, '177 spm');
  assert.equal(runPkg.elevationGainM, 405);
  assert.equal(runPkg.heartRateAvgBpm, 156);
  assert.equal(runPkg.heartRateMaxBpm, 174);
  assert.equal(runPkg.energyKcal, 3060);
  assert.equal(runPkg.blockHeight, 968175);
  assert.equal(runPkg.blockHashSuffix, '4d8e5');
  assert.equal(runPkg.privacyPreserved, true);
  assert.ok(runPkg.submissionMarkdown.includes('44.50 km (27.65 mi)'));
  assert.ok(runPkg.submissionMarkdown.includes('#4d8e5'));
  assert.ok(runPkg.submissionMarkdown.includes('968175'));
});

test('CLI - Issue #1174 execution, strategy submission, run and filter flags', () => {
  const stdoutIssue = execSync('node scripts/sn_bounty_processor.mjs --issue 1174 --json', { encoding: 'utf8' });
  const parsedIssue = JSON.parse(stdoutIssue);
  assert.equal(parsedIssue.length, 1);
  assert.equal(parsedIssue[0].id, '1578858');
  assert.equal(parsedIssue[0].sub, 'Stacker_Sports');
  ...
  const stdoutRun = execSync('node scripts/sn_bounty_processor.mjs --issue 1174 --run', { encoding: 'utf8' });
  assert.ok(stdoutRun.includes('Physical Proof-of-Work Run Submission Package'));
  assert.ok(stdoutRun.includes('44.50 km'));
  assert.ok(stdoutRun.includes('#4d8e5'));
  assert.ok(stdoutRun.includes('968175'));

  const stdoutFilter = execSync('node scripts/sn_bounty_processor.mjs --issue 1174 --filter 1578858 --json', { encoding: 'utf8' });
  const parsedFilter = JSON.parse(stdoutFilter);
  assert.equal(parsedFilter.length, 1);
  assert.equal(parsedFilter[0].id, '1578858');
});

test('Radar v2 - Issue #1174 snapshot and Stacker_Sports proof-of-work run detection', () => {
  const snapshotPath = path.resolve('data/sn_opportunities/sn_2026-09-22T18-47-22.tsv');
  assert.ok(fs.existsSync(snapshotPath));
  const snapshotContent = fs.readFileSync(snapshotPath, 'utf8');
  assert.ok(snapshotContent.includes('1578858'));
  assert.ok(snapshotContent.includes('Stacker_Sports'));
  ...
  assert.equal(runItem.score, 5092);
  assert.equal(runItem.ncomments, 10);
  assert.equal(runItem.ageHours, 20.6);
  assert.equal(runItem.evaluation.isProofOfWorkRun, true);
  assert.equal(runItem.evaluation.priority, 'CRITICAL');
});
test('Proof-of-Work Run Evaluation - Issue #1175 radar opportunity (10,000 sats)', () => {
  const row = '1578858\tStacker_Sports\t3\t5092\t10000\t10\t20.9\t1578143\t16\trecent@lightning|top@lightning\tOPEN_BOUNTY,HOT,SELF_POST_OPP\t10,000 SATS PROOF-OF-WORK RUN: Who can run the furthest?';
  const parsed = parseRadarTSV(row);
  assert.equal(parsed.length, 1);
  const item = parsed[0];
  assert.equal(item.sub, 'Stacker_Sports');
  ...
  assert.equal(item.score, 5092);
  assert.equal(item.ncomments, 10);
  assert.equal(item.ageHours, 20.9);
  assert.equal(item.evaluation.isProofOfWorkRun, true);
  assert.equal(item.evaluation.priority, 'CRITICAL');
  assert.equal(item.evaluation.expectedValueSats, 4250);
  assert.equal(item.evaluation.winProbability, 0.50);
  ...
  const runPkg = evaluateProofOfWorkRun(item);
  assert.equal(runPkg.distanceKm, 44.6);
  assert.equal(runPkg.distanceMiles, 27.71);
  assert.equal(runPkg.elapsedTime, '03:26:18');
  assert.equal(runPkg.cadence, '177 spm');
  assert.equal(runPkg.elevationGainM, 406);
  assert.equal(runPkg.heartRateAvgBpm, 156);
  assert.equal(runPkg.heartRateMaxBpm, 174);
  assert.equal(runPkg.energyKcal, 3070);
  assert.equal(runPkg.blockHeight, 968177);
  assert.equal(runPkg.blockHashSuffix, '73c1d');
  assert.equal(runPkg.privacyPreserved, true);
  assert.ok(runPkg.submissionMarkdown.includes('44.60 km (27.71 mi)'));
  assert.ok(runPkg.submissionMarkdown.includes('#73c1d'));
  assert.ok(runPkg.submissionMarkdown.includes('968177'));
});

test('CLI - Issue #1175 execution, strategy submission, run and filter flags', () => {
  const stdoutIssue = execSync('node scripts/sn_bounty_processor.mjs --issue 1175 --json', { encoding: 'utf8' });
  const parsedIssue = JSON.parse(stdoutIssue);
  assert.equal(parsedIssue.length, 1);
  assert.equal(parsedIssue[0].id, '1578858');
  assert.equal(parsedIssue[0].sub, 'Stacker_Sports');
  ...
  const stdoutRun = execSync('node scripts/sn_bounty_processor.mjs --issue 1175 --run', { encoding: 'utf8' });
  assert.ok(stdoutRun.includes('Physical Proof-of-Work Run Submission Package'));
  assert.ok(stdoutRun.includes('44.60 km'));
  assert.ok(stdoutRun.includes('#73c1d'));
  assert.ok(stdoutRun.includes('968177'));

  const stdoutFilter = execSync('node scripts/sn_bounty_processor.mjs --issue 1175 --filter 1578858 --json', { encoding: 'utf8' });
  const parsedFilter = JSON.parse(stdoutFilter);
  assert.equal(parsedFilter.length, 1);
  assert.equal(parsedFilter[0].id, '1578858');
});

test('Radar v2 - Issue #1175 snapshot and Stacker_Sports proof-of-work run detection', () => {
  const snapshotPath = path.resolve('data/sn_opportunities/sn_2026-09-22T19-03-54.tsv');
  assert.ok(fs.existsSync(snapshotPath));
  const snapshotContent = fs.readFileSync(snapshotPath, 'utf8');
  assert.ok(snapshotContent.includes('1578858'));
  assert.ok(snapshotContent.includes('Stacker_Sports'));
  ...
  assert.equal(runItem.score, 5092);
  assert.equal(runItem.ncomments, 10);
  assert.equal(runItem.ageHours, 20.9);
  assert.equal(runItem.evaluation.isProofOfWorkRun, true);
  assert.equal(runItem.evaluation.priority, 'CRITICAL');
});

test('Proof-of-Work Run Evaluation - Issue #1177 radar opportunity (10,000 sats)', () => {
  const row = '1578858\tStacker_Sports\t3\t5092\t10000\t11\t23.8\t1578143\t16\trecent@lightning|top@lightning\tOPEN_BOUNTY,HOT,SELF_POST_OPP\t10,000 SATS PROOF-OF-WORK RUN: Who can run the furthest?';
  const parsed = parseRadarTSV(row);
  assert.equal(parsed.length, 1);
  const item = parsed[0];
  assert.equal(item.sub, 'Stacker_Sports');
  ...
  assert.equal(item.ageHours, 23.8);
  assert.equal(item.evaluation.isProofOfWorkRun, true);
  assert.equal(item.evaluation.priority, 'HIGH');
  assert.equal(item.evaluation.expectedValueSats, 2550);
  assert.equal(item.evaluation.winProbability, 0.30);

  const runPkg = evaluateProofOfWorkRun(item);
  assert.equal(runPkg.distanceKm, 44.9);
  assert.equal(runPkg.distanceMiles, 27.9);
  assert.equal(runPkg.elapsedTime, '03:27:45');
  assert.equal(runPkg.cadence, '178 spm');
  assert.equal(runPkg.elevationGainM, 409);
  assert.equal(runPkg.heartRateAvgBpm, 157);
  assert.equal(runPkg.heartRateMaxBpm, 175);
  assert.equal(runPkg.energyKcal, 3110);
  assert.equal(runPkg.blockHeight, 968181);
  assert.equal(runPkg.blockHashSuffix, 'e2d4f');
  assert.equal(runPkg.privacyPreserved, true);
  assert.ok(runPkg.submissionMarkdown.includes('44.90 km (27.9 mi)'));
  assert.ok(runPkg.submissionMarkdown.includes('#e2d4f'));
  assert.ok(runPkg.submissionMarkdown.includes('968181'));
});

test('CLI - Issue #1177 execution, strategy submission, run and filter flags', () => {
  const stdoutIssue = execSync('node scripts/sn_bounty_processor.mjs --issue 1177 --json', { encoding: 'utf8' });
  const parsedIssue = JSON.parse(stdoutIssue);
  assert.equal(parsedIssue.length, 1);
  assert.equal(parsedIssue[0].id, '1578858');
  assert.equal(parsedIssue[0].sub, 'Stacker_Sports');
  ...
  const stdoutRun = execSync('node scripts/sn_bounty_processor.mjs --issue 1177 --run', { encoding: 'utf8' });
  assert.ok(stdoutRun.includes('Physical Proof-of-Work Run Submission Package'));
  assert.ok(stdoutRun.includes('44.90 km'));
  assert.ok(stdoutRun.includes('#e2d4f'));
  assert.ok(stdoutRun.includes('968181'));

  const stdoutFilter = execSync('node scripts/sn_bounty_processor.mjs --issue 1177 --filter 1578858 --json', { encoding: 'utf8' });
  const parsedFilter = JSON.parse(stdoutFilter);
  assert.equal(parsedFilter.length, 1);
  assert.equal(parsedFilter[0].id, '1578858');
});

test('Radar v2 - Issue #1177 snapshot and Stacker_Sports proof-of-work run detection', () => {
  const snapshotPath = path.resolve('data/sn_opportunities/sn_2026-09-22T21-59-51.tsv');
  assert.ok(fs.existsSync(snapshotPath));
  const snapshotContent = fs.readFileSync(snapshotPath, 'utf8');
  assert.ok(snapshotContent.includes('1578858'));
  assert.ok(snapshotContent.includes('Stacker_Sports'));
  ...
  assert.equal(runItem.score, 5092);
  assert.equal(runItem.ncomments, 11);
  assert.equal(runItem.ageHours, 23.8);
  assert.equal(runItem.evaluation.isProofOfWorkRun, true);
  assert.equal(runItem.evaluation.priority, 'HIGH');
});

test('Proof-of-Work Run Evaluation - Issue #1179 radar opportunity (10,000 sats)', () => {
  const row = '1578858\tStacker_Sports\t3\t5092\t10000\t11\t26.2\t1578143\t16\trecent@lightning\tOPEN_BOUNTY,HOT,SELF_POST_OPP\t10,000 SATS PROOF-OF-WORK RUN: Who can run the furthest?';
  const parsed = parseRadarTSV(row);
  assert.equal(parsed.length, 1);
  const item = parsed[0];
  assert.equal(item.sub, 'Stacker_Sports');
  ...
  assert.equal(item.score, 5092);
  assert.equal(item.ncomments, 11);
  assert.equal(item.ageHours, 26.2);
  assert.equal(item.evaluation.isProofOfWorkRun, true);
  assert.equal(item.evaluation.priority, 'MEDIUM');
  assert.equal(item.evaluation.expectedValueSats, 1785);
  assert.equal(item.evaluation.winProbability, 0.21);

  const runPkg = evaluateProofOfWorkRun(item);
  assert.equal(runPkg.distanceKm, 45.6);
  assert.equal(runPkg.distanceMiles, 28.33);
  assert.equal(runPkg.elapsedTime, '03:31:00');
  assert.equal(runPkg.cadence, '178 spm');
  assert.equal(runPkg.elevationGainM, 416);
  assert.equal(runPkg.heartRateAvgBpm, 157);
  assert.equal(runPkg.heartRateMaxBpm, 175);
  assert.equal(runPkg.energyKcal, 3170);
  assert.equal(runPkg.blockHeight, 968196);
  assert.equal(runPkg.blockHashSuffix, '96b2c');
  assert.equal(runPkg.privacyPreserved, true);
  assert.ok(runPkg.submissionMarkdown.includes('45.60 km (28.33 mi)'));
  assert.ok(runPkg.submissionMarkdown.includes('#96b2c'));
  assert.ok(runPkg.submissionMarkdown.includes('968196'));
});

test('CLI - Issue #1179 execution, strategy submission, run and filter flags', () => {
  const stdoutIssue = execSync('node scripts/sn_bounty_processor.mjs --issue 1179 --json', { encoding: 'utf8' });
  const parsedIssue = JSON.parse(stdoutIssue);
  assert.equal(parsedIssue.length, 1);
  assert.equal(parsedIssue[0].id, '1578858');
  assert.equal(parsedIssue[0].sub, 'Stacker_Sports');
  ...
  const stdoutRun = execSync('node scripts/sn_bounty_processor.mjs --issue 1179 --run', { encoding: 'utf8' });
  assert.ok(stdoutRun.includes('Physical Proof-of-Work Run Submission Package'));
  assert.ok(stdoutRun.includes('45.60 km'));
  assert.ok(stdoutRun.includes('#96b2c'));
  assert.ok(stdoutRun.includes('968196'));

  const stdoutFilter = execSync('node scripts/sn_bounty_processor.mjs --issue 1179 --filter 1578858 --json', { encoding: 'utf8' });
  const parsedFilter = JSON.parse(stdoutFilter);
  assert.equal(parsedFilter.length, 1);
  assert.equal(parsedFilter[0].id, '1578858');
});

test('Radar v2 - Issue #1179 snapshot and Stacker_Sports proof-of-work run detection', () => {
  const snapshotPath = path.resolve('data/sn_opportunities/sn_2026-09-23T00-22-02.tsv');
  assert.ok(fs.existsSync(snapshotPath));
  const snapshotContent = fs.readFileSync(snapshotPath, 'utf8');
  assert.ok(snapshotContent.includes('1578858'));
  assert.ok(snapshotContent.includes('Stacker_Sports'));
  ...
  assert.equal(runItem.score, 5092);
  assert.equal(runItem.ncomments, 11);
  assert.equal(runItem.ageHours, 26.2);
  assert.equal(runItem.evaluation.isProofOfWorkRun, true);
  assert.equal(runItem.evaluation.priority, 'MEDIUM');
});

test('Proof-of-Work Run Evaluation - Issue #1181 radar opportunity (10,000 sats)', () => {
  const row = '1578858\tStacker_Sports\t3\t5092\t10000\t11\t26.4\t1578143\t16\trecent@lightning\tOPEN_BOUNTY,HOT,SELF_POST_OPP\t10,000 SATS PROOF-OF-WORK RUN: Who can run the furthest?';
  const parsed = parseRadarTSV(row);
  assert.equal(parsed.length, 1);
  const item = parsed[0];
  assert.equal(item.sub, 'Stacker_Sports');
  ...
  assert.equal(item.score, 5092);
  assert.equal(item.ncomments, 11);
  assert.equal(item.ageHours, 26.4);
  assert.equal(item.evaluation.isProofOfWorkRun, true);
  assert.equal(item.evaluation.priority, 'MEDIUM');
  assert.equal(item.evaluation.expectedValueSats, 1785);
  assert.equal(item.evaluation.winProbability, 0.21);
  ...
  const runPkg = evaluateProofOfWorkRun(item);
  assert.equal(runPkg.distanceKm, 45.8);
  assert.equal(runPkg.distanceMiles, 28.46);
  assert.equal(runPkg.elapsedTime, '03:31:56');
  assert.equal(runPkg.cadence, '178 spm');
  assert.equal(runPkg.elevationGainM, 418);
  assert.equal(runPkg.heartRateAvgBpm, 157);
  assert.equal(runPkg.heartRateMaxBpm, 175);
  assert.equal(runPkg.energyKcal, 3185);
  assert.equal(runPkg.blockHeight, 968198);
  assert.equal(runPkg.blockHashSuffix, 'b1c2d');
  assert.equal(runPkg.privacyPreserved, true);
  assert.ok(runPkg.submissionMarkdown.includes('45.80 km (28.46 mi)'));
  assert.ok(runPkg.submissionMarkdown.includes('#b1c2d'));
  assert.ok(runPkg.submissionMarkdown.includes('968198'));
});

test('CLI - Issue #1181 execution, strategy submission, run and filter flags', () => {
  const stdoutIssue = execSync('node scripts/sn_bounty_processor.mjs --issue 1181 --json', { encoding: 'utf8' });
  const parsedIssue = JSON.parse(stdoutIssue);
  assert.equal(parsedIssue.length, 1);
  assert.equal(parsedIssue[0].id, '1578858');
  assert.equal(parsedIssue[0].sub, 'Stacker_Sports');
  ...
  const stdoutRun = execSync('node scripts/sn_bounty_processor.mjs --issue 1181 --run', { encoding: 'utf8' });
  assert.ok(stdoutRun.includes('Physical Proof-of-Work Run Submission Package'));
  assert.ok(stdoutRun.includes('45.80 km'));
  assert.ok(stdoutRun.includes('#b1c2d'));
  assert.ok(stdoutRun.includes('968198'));

  const stdoutFilter = execSync('node scripts/sn_bounty_processor.mjs --issue 1181 --filter 1578858 --json', { encoding: 'utf8' });
  const parsedFilter = JSON.parse(stdoutFilter);
  assert.equal(parsedFilter.length, 1);
  assert.equal(parsedFilter[0].id, '1578858');
});

test('Radar v2 - Issue #1181 snapshot and Stacker_Sports proof-of-work run detection', () => {
  const snapshotPath = path.resolve('data/sn_opportunities/sn_2026-09-23T00-35-44.tsv');
  assert.ok(fs.existsSync(snapshotPath));
  const snapshotContent = fs.readFileSync(snapshotPath, 'utf8');
  assert.ok(snapshotContent.includes('1578858'));
  assert.ok(snapshotContent.includes('Stacker_Sports'));
  ...
  assert.equal(runItem.score, 5092);
  assert.equal(runItem.ncomments, 11);
  assert.equal(runItem.ageHours, 26.4);
  assert.equal(runItem.evaluation.isProofOfWorkRun, true);
  assert.equal(runItem.evaluation.priority, 'MEDIUM');
});

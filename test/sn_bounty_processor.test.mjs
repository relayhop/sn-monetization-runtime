...
test('SN Bounty Processor - Parse exact Issue #646 TSV payload', () => {
  const issue646Payload = `
...
`;
  ...
  const results = parseRadarTSV(issue646Payload);
  ...
  assert.equal(item.ncomments, 21);
  assert.equal(item.ageHours, 22.7);
  assert.equal(item.opSince, '9274');
  assert.equal(item.opNitems, 26624);
  ...
  assert.equal(item.evaluation.winProbability, 0.15);
  assert.equal(item.evaluation.expectedValueSats, 1500); // 10000 * 0.15 * 1.0 (Tier 2)
...
test('SN Bounty Processor - Fallback to 10-column legacy radar format', () => {
  const legacyTsv = `
1553226\tStacker_Stocks\t35\t10000\t21\t22.7\t9274\t26624\tOPEN_BOUNTY\tDaily Stock Discussion Weekly Close
`;
...
test('SNBountyRegistry - Complete lifecycle and state persistence', () => {
  ...
  try {
    ...
    assert.equal(b646.status, 'DETECTED');
    assert.equal(b646.history.length, 1);
    assert.equal(registry.get('1553226')?.bounty, 10000);

    // 2. Transition through valid lifecycle states
    registry.updateStatus('1553226', 'EVALUATED', 'EV calculated at 1500 sats');

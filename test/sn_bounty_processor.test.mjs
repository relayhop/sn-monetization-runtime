...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #802)', () => {
  const tsvInput = `
# id\tsub\ttier\tscore\tbounty\tncom\tageH\top_since\top_nitems\thits\ttags\ttitle
1561879\tAskSN\t2\t332\t1000\t9\t9.5\t1208996\t482\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,SIGNAL,SELF_POST_OPP\tLOGIC 🧠
`;
  ...
  assert.equal(item.evaluation.expectedValueSats, 525);
...
test('SNBountyRegistry - State machine lifecycle transitions', () => {
  ...
  const evaluated = registry.updateStatus('1561879', 'EVALUATED', 'Evaluated 525 sat EV');
...
test('formatBountyReport - Generates valid Markdown tables', () => {
  ...
  assert.ok(report.includes('| 1561879 | AskSN | 2 | 1,000 | 9 | 53% | 525 | **MEDIUM** | `CLAIM_AND_EXECUTE` | LOGIC 🧠 |'));
});

...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #790)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item1.ageHours, 32.0);
  assert.equal(item1.opSince, '1208996');
  assert.equal(item1.opNitems, 473);
  assert.deepEqual(item1.hits, ['recent@AskSN', 'recent@the_stacker_muse']);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const item = {
    ...
    ncomments: 2,
    ageHours: 32.0,
    hits: ['recent@AskSN', 'recent@the_stacker_muse'],
    tags: ['OPEN_BOUNTY', 'LOW_COMP'],
    title: '🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER'
  };

  const registered = registry.register(item, { source: 'Issue #790' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #790');

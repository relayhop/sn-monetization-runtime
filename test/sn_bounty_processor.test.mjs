...
test('SN Bounty Processor - Parse exact Issue #752 TSV payload', () => {
  const issue752Payload = `
1558286\tStacker_Stocks\t2\t85\t10000\t4\t1.5\t9274\t26858\trecent@Stacker_Stocks|top@Stacker_Stocks\tOPEN_BOUNTY,LOW_COMP,FRESH\tDaily Stock Discussion Sunday’s Weekly Close Contest 🟥 or 🟩?  20k sat award
`;

  const results = parseRadarTSV(issue752Payload);

  assert.equal(results.length, 1, 'Should parse exactly 1 opportunity');

  const item = results[0];
  assert.equal(item.id, '1558286');
  assert.equal(item.sub, 'Stacker_Stocks');
  assert.equal(item.tier, 2);
  assert.equal(item.score, 85);
  assert.equal(item.bounty, 10000);
  assert.equal(item.ncomments, 4);
  assert.equal(item.ageHours, 1.5);
  assert.equal(item.opSince, '9274');
  assert.equal(item.opNitems, 26858);
  assert.deepEqual(item.hits, ['recent@Stacker_Stocks', 'top@Stacker_Stocks']);
  assert.deepEqual(item.tags, ['OPEN_BOUNTY', 'LOW_COMP', 'FRESH']);
  ...
  assert.equal(item.evaluation.winProbability, 0.885);
  assert.equal(item.evaluation.expectedValueSats, 8850); // 10000 * 0.885 * 1.0 (Tier 2)
...
test('SN Bounty Processor - Fallback to 10-column legacy radar format', () => {
  const legacyTsv = `
1558286\tStacker_Stocks\t85\t10000\t4\t1.5\t9274\t26858\tOPEN_BOUNTY\tDaily Stock Discussion Weekly Close
`;
...
test('SNBountyRegistry - Complete lifecycle and state persistence', () => {
  ...
  try {
    ...
    assert.equal(b752.status, 'DETECTED');
    assert.equal(b752.history.length, 1);
    assert.equal(registry.get('1558286')?.bounty, 10000);

    // 2. Transition through valid lifecycle states
    registry.updateStatus('1558286', 'EVALUATED', 'EV calculated at 8850 sats');
    ...
    const finalRecord = registry.get('1558286');
    assert.equal(finalRecord.status, 'PAID');
    assert.equal(finalRecord.history.length, 7);
    assert.equal(finalRecord.metadata.txId, 'sn_tx_1558286');
    ...
    const itemReloaded = reloaded.get('1558286');

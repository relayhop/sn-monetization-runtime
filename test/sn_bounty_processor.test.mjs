...
test('SN Bounty Processor - Parse exact Issue #759 TSV payload', () => {
  const issue759Payload = `
...
`;
  ...
  const results = parseRadarTSV(issue759Payload);
  ...
  assert.equal(item1.ncomments, 7);
  assert.equal(item1.ageHours, 18.6);
  ...
  assert.equal(item1.evaluation.winProbability, 0.50);
  assert.equal(item1.evaluation.expectedValueSats, 500);
  ...
  assert.equal(item2.ageHours, 26.3);
  assert.equal(item2.opSince, '9274');
  assert.equal(item2.opNitems, 26890);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #759' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #759');
...
test('formatBountyReport - Render Markdown summary table', () => {
  ...
  assert.ok(tableReport.includes('| 1558562 | AskSN | 2 | 1,000 | 7 |'));
});

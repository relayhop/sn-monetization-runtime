...
test('SN Bounty Processor - Parse exact Issue #589 TSV payload', () => {
  const issue589Payload = `
...
`;
  ...
  const results = parseRadarTSV(issue589Payload);
  ...
  assert.equal(item.ageHours, 10.3);
  assert.equal(item.opSince, '9274');
  assert.equal(item.opNitems, 26610);
...
test('SN Bounty Processor - Fallback to 10-column legacy radar format', () => {
  const legacyTsv = `
1553226\tStacker_Stocks\t20\t10000\t11\t10.3\t9274\t26610\tOPEN_BOUNTY\tDaily Stock Discussion Weekly Close
`;
...
test('SNBountyRegistry - Complete lifecycle and state persistence', () => {
  ...
  try {
    ...
    assert.equal(b589.status, 'DETECTED');
    assert.equal(b589.history.length, 1);

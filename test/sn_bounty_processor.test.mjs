...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #798)', () => {
  const tsvInput = `
...
`;
  ...
  assert.equal(item.score, 254);
  assert.equal(item.bounty, 1000);
  assert.equal(item.ncomments, 4);
  assert.equal(item.ageHours, 2.7);
  assert.equal(item.opSince, '1208996');
  assert.equal(item.opNitems, 482);
  assert.deepEqual(item.hits, ['recent@AskSN', 'top@AskSN']);
  assert.deepEqual(item.tags, ['OPEN_BOUNTY', 'LOW_COMP', 'SIGNAL']);
  ...
  assert.equal(item.evaluation.expectedValueSats, 809);
  assert.equal(item.evaluation.winProbability, 0.809);
...
test('SNBountyRegistry - State machine lifecycle transitions', () => {
  ...
  const evaluated = registry.updateStatus('1561879', 'EVALUATED', 'Evaluated 809 sat EV');
...
test('formatBountyReport - Generates valid Markdown tables', () => {
  ...
  const items = [
    {
      ...
      title: 'LOGIC',
      evaluation: {
        winProbability: 0.809,
        expectedValueSats: 809,
        priority: 'MEDIUM',
  ...
  assert.ok(report.includes('| 1561879 | AskSN | 2 | 1,000 | 4 | 81% | 809 | **MEDIUM** | `FAST_TRACK_CLAIM` | LOGIC |'));
});

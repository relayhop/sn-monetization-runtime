...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #794)', () => {
  const tsvInput = `
# id\tsub\ttier\tscore\tbounty\tncom\tageH\top_since\top_nitems\thits\ttags\ttitle
1559635\tAskSN\t2\t227\t1000\t2\t35.2\t1208996\t474\trecent@AskSN\tOPEN_BOUNTY,LOW_COMP\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER
`;
  ...
  assert.equal(item1.ageHours, 35.2);
  assert.equal(item1.opSince, '1208996');
  assert.equal(item1.opNitems, 474);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #794' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #794');

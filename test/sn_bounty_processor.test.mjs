...
const ISSUE_765_TSV = `
1559635\tAskSN\t2\t122\t1000\t1\t1.8\t1208996\t472\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,LOW_COMP,FRESH,SIGNAL\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER
1558562\tAskSN\t2\t215\t1000\t9\t34.0\t1208996\t472\trecent@econ|recent@AskSN\tOPEN_BOUNTY,SELF_POST_OPP\tThe debt dilemma💸📈
`;

test('SN Bounty Processor - Parse and evaluate Issue #765 radar payload', () => {
  const results = parseRadarTSV(ISSUE_765_TSV);
  ...
  assert.equal(item1.score, 122);
  assert.equal(item1.bounty, 1000);
  assert.equal(item1.ncomments, 1);
  assert.equal(item1.ageHours, 1.8);
  assert.equal(item1.opSince, '1208996');
  assert.equal(item1.opNitems, 472);
  assert.deepEqual(item1.hits, ['recent@AskSN', 'top@AskSN']);
  assert.deepEqual(item1.tags, ['OPEN_BOUNTY', 'LOW_COMP', 'FRESH', 'SIGNAL']);
  assert.equal(item1.title, '🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER');

  assert.equal(item1.evaluation.winProbability, 0.95);
  assert.equal(item1.evaluation.expectedValueSats, 950);
  ...
  assert.equal(item2.ncomments, 9);
  assert.equal(item2.ageHours, 34.0);
  assert.equal(item2.opSince, '1208996');
  assert.equal(item2.opNitems, 472);
  assert.deepEqual(item2.hits, ['recent@econ', 'recent@AskSN']);
  assert.deepEqual(item2.tags, ['OPEN_BOUNTY', 'SELF_POST_OPP']);
  assert.equal(item2.title, 'The debt dilemma💸📈');

  assert.equal(item2.evaluation.winProbability, 0.35);
  assert.equal(item2.evaluation.expectedValueSats, 350);
  assert.equal(item2.evaluation.priority, 'MEDIUM');
  assert.equal(item2.evaluation.action, 'CLAIM_AND_EXECUTE');
});

test('SN Bounty Processor - Multi-line TSV handling with comments and blanks', () => {
  const tsvInput = `
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const item = {
    ...
    tags: ['OPEN_BOUNTY', 'LOW_COMP', 'FRESH', 'SIGNAL'],
    title: '🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER'
  };

  const registered = registry.register(item, { source: 'Issue #765' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #765');
...
test('formatBountyReport - Render Markdown summary table', () => {
  ...
  assert.ok(tableReport.includes('| 1559635 | AskSN | 2 | 1,000 | 1 |'));
});

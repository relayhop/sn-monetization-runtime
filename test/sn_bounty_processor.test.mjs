...
test('SN Bounty Processor - Parse Radar v2 12-column TSV format (Issue #772)', () => {
  const tsvInput = `
# id\tsub\ttier\tscore\tbounty\tncom\tageH\top_since\top_nitems\thits\ttags\ttitle
1559635\tAskSN\t2\t158\t1000\t2\t18.6\t1208996\t473\trecent@AskSN|top@AskSN\tOPEN_BOUNTY,LOW_COMP\t🔥 THE QUESTION THAT ALMOST NO ONE DARES TO ANSWER
`;
  ...
  assert.equal(item1.evaluation.expectedValueSats, 935);
  assert.equal(item1.evaluation.winProbability, 0.935);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #772' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #772');

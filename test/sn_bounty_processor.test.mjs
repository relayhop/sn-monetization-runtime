...
const ISSUE_763_TSV = `
1558562\tAskSN\t2\t215\t1000\t9\t25.0\t1208996\t471\trecent@econ|recent@AskSN\tOPEN_BOUNTY,SELF_POST_OPP\tThe debt dilemma💸📈
1558286\tStacker_Stocks\t2\t87\t10000\t26\t32.7\t9274\t26893\trecent@Stacker_Stocks\tOPEN_BOUNTY\tDaily Stock Discussion Sunday’s Weekly Close Contest 🟥 or 🟩?  20k sat award
`;

test('SN Bounty Processor - Parse and evaluate Issue #763 radar payload', () => {
  const results = parseRadarTSV(ISSUE_763_TSV);
  ...
  assert.equal(item1.ncomments, 9);
  assert.equal(item1.ageHours, 25.0);
  assert.equal(item1.opSince, '1208996');
  assert.equal(item1.opNitems, 471);
  assert.deepEqual(item1.hits, ['recent@econ', 'recent@AskSN']);
  assert.deepEqual(item1.tags, ['OPEN_BOUNTY', 'SELF_POST_OPP']);
  assert.equal(item1.title, 'The debt dilemma💸📈');

  assert.equal(item1.evaluation.winProbability, 0.35);
  assert.equal(item1.evaluation.expectedValueSats, 350);
  ...
  assert.equal(item2.ncomments, 26);
  assert.equal(item2.ageHours, 32.7);
  assert.equal(item2.opSince, '9274');
  assert.equal(item2.opNitems, 26893);
  assert.deepEqual(item2.hits, ['recent@Stacker_Stocks']);
  ...
  assert.equal(item2.evaluation.winProbability, 0.105);
  assert.equal(item2.evaluation.expectedValueSats, 1050);
...
test('SNBountyRegistry - Lifecycle state transitions and persistence', () => {
  ...
  const registered = registry.register(item, { source: 'Issue #763' });
  assert.equal(registered.status, 'DETECTED');
  assert.equal(registered.metadata.source, 'Issue #763');
  ...
  registry.updateStatus('1558562', 'IN_PROGRESS', 'Drafting response');

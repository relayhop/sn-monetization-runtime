...
export function evaluateOpportunity(item) {
  ...
  const isSportsPickEm = titleLower.includes('pick em') || titleLower.includes('pick \'em') || titleLower.includes('afl') || sub === 'Stacker_Sports';
...
export function evaluateSportsPickEm(item, customData = {}) {
  ...
  const defaultLeague = isRandomSports
    ? 'Multi-Sport Cross-League Selection'
    : 'AFL (Australian Football League)';

  const defaultRound = isRandomSports
    ? 'Weekly Random Sports Slate'
    : 'Finals Week 2 (Semi Finals)';

  const defaultFixtures = isRandomSports
    ? [
        {
          match: 'NFL: Baltimore Ravens vs Kansas City Chiefs',
          venue: 'Arrowhead Stadium',
          prediction: 'Kansas City Chiefs by 4 points',
          margin: '1-6 pts',
          firstGoalscorer: 'Travis Kelce',
          rationale: 'Red-zone efficiency and home defensive turnover edge in late-game drives.'
        },
        {
          match: 'Premier League: Arsenal vs Brighton',
          venue: 'Emirates Stadium',
          prediction: 'Arsenal by 2 goals',
          margin: '2-0',
          firstGoalscorer: 'Bukayo Saka',
          rationale: 'High progressive pass completion and dominance in defensive transition phases.'
        },
        {
          match: 'College Football: Texas vs Michigan',
          venue: 'Michigan Stadium',
          prediction: 'Texas Longhorns by 7 points',
          margin: '7-10 pts',
          firstGoalscorer: 'Quinn Ewers (passing)',
          rationale: 'Quarterback pocket stability against heavy blitz packages and perimeter speed advantage.'
        },
        {
          match: 'AFL: Port Adelaide Power vs Hawthorn Hawks',
          venue: 'Adelaide Oval',
          prediction: 'Hawthorn Hawks by 14 points',
          margin: '11-20 pts',
          firstGoalscorer: 'Nick Watson',
          rationale: 'Hawthorn transitions cleanly through corridor transition; Port defense vulnerable to turnover counters.'
        }
      ]
    : [
        {
...
- Key Contributor / First Score: ${f.firstGoalscorer}
- Tactical Analysis: ${f.rationale}`;
  }).join('\n\n');

  const submissionMarkdown = `## ${roundName} Pick'Em Submission

**Opportunity Reference:** Item #${item.id} (${item.title || 'Sports Pick Em'})
...
export function evaluateSelfPostOpportunity(item, postContext = {}) {
  ...
  const defaultHookTitle = isSports
  ...
  const defaultCallToAction = isSports
...
if (isMain) {
  ...
  if (rowArgIdx !== -1 && args[rowArgIdx + 1]) {
    ...
    tsvContent = '1562157\tStacker_Sports\t3\t1112\t2100\t9\t20.0\t232181\t4050\trecent@Stacker_Sports|top@Stacker_Sports\tOPEN_BOUNTY,HOT,SELF_POST_OPP\tWeekly Random Sports Pick \'em\n1561879\tAskSN\t2\t422\t1000\t11\t32.8\t1208996\t484\trecent@AskSN\tOPEN_BOUNTY,SELF_POST_OPP\tLOGIC 🧠';
  }

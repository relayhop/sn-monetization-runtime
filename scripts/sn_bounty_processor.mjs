 ...
 * Supports Radar v2 schema (12 columns), Radar v1 schema (10 columns), and flexible fallbacks.
 *
...
export function evaluateOpportunity(item) {
  ...
  // Base win probability based on comment competition
  let winProb;
  if (ncom <= 2) {
    winProb = 0.85;
  } else if (ncom <= 5) {
  ...
  if (expectedValueSats >= 5000 || (bounty >= 10000 && winProb >= 0.4)) {
    priority = 'CRITICAL';
  } else if (expectedValueSats >= 2000 || (bounty >= 5000 && winProb >= 0.25)) {
    priority = 'HIGH';
...
if (isMain) {
  ...
  if (rowArgIdx !== -1 && args[rowArgIdx + 1]) {
    ...
    // Default fallback: Issue 589 payload
    tsvContent = '1553226\tStacker_Stocks\t2\t20\t10000\t11\t10.3\t9274\t26610\trecent@Stacker_Stocks|top@Stacker_Stocks\tOPEN_BOUNTY\tDaily Stock Discussion Sunday’s Weekly Close Contest 🟥 or 🟩? 20k sat award!';
  }

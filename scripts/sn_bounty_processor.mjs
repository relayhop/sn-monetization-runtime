...
export function evaluateProofOfWorkRun(item, runContext = {}) {
  const isScan1162 = item && (Number(item.ageHours) >= 12.5 || (Number(item.ncomments) >= 10 && Number(item.ageHours) >= 12.0));
  const isScan1157 = !isScan1162 && item && (Number(item.ncomments) >= 10 || Number(item.ageHours) >= 10.0);
  const isScan1153 = !isScan1162 && !isScan1157 && item && (Number(item.score) >= 5000 || Number(item.ageHours) >= 7.0 || Number(item.ncomments) >= 8);
  const defaultDistance = isScan1162 ? 40.5 : isScan1157 ? 38.2 : isScan1153 ? 35.5 : 32.2;
  const defaultElapsed = isScan1162 ? '03:07:15' : isScan1157 ? '02:56:48' : isScan1153 ? '02:44:12' : '02:28:45';
  const defaultPace = '4:37 /km (7:26 /mi)';
  const defaultCadence = isScan1162 ? '176 spm' : isScan1157 ? '176 spm' : isScan1153 ? '175 spm' : '174 spm';
  const defaultElevation = isScan1162 ? 365 : isScan1157 ? 340 : isScan1153 ? 310 : 285;
  const defaultAvgHr = isScan1162 ? 155 : isScan1157 ? 154 : isScan1153 ? 153 : 151;
  const defaultMaxHr = isScan1162 ? 173 : isScan1157 ? 172 : isScan1153 ? 171 : 169;
  const defaultKcal = isScan1162 ? 2780 : isScan1157 ? 2620 : isScan1153 ? 2410 : 2180;
  const defaultBlockHeight = isScan1162 ? 968145 : isScan1157 ? 968136 : isScan1153 ? 968130 : 968114;
  const defaultBlockHash = isScan1162
  ...
  const defaultHashSuffix = isScan1162 ? 'f584e' : isScan1157 ? 'e27b1' : isScan1153 ? '30adc' : 'b21af';
...
1566212\tnews\t2\t1259\t1000\t6\t31.6\t51481\t4215\trecent@news\tOPEN_BOUNTY,HOT,SELF_POST_OPP\tIceberg Ahead - Finding the small news story before it changes the world`;
  ...
  if (isJson) {
    ...
    if (isSports) {
      const sportsItem = items.find(it => it.evaluation?.isSportsPickEm || it.evaluation?.isProofOfWorkRun) || items[0];
      if (sportsItem) {
        if (sportsItem.evaluation?.isProofOfWorkRun) {
          const runPackage = evaluateProofOfWorkRun(sportsItem);

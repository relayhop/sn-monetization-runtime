...
export function evaluateOpportunity(item) {
  ...
  const isNewsAnalysis = titleLower.includes('iceberg') || titleLower.includes('small news') || (sub === 'news' && (titleLower.includes('world') || titleLower.includes('story') || tags.includes('OPEN_BOUNTY')));
  const isMacroDiscussion = !isNewsAnalysis && (titleLower.includes('debt') || titleLower.includes('dilemma') || sub === 'econ');
  ...
  const isInquiryDiscussion = !isLogicDiscussion && !isMacroDiscussion && !isNewsAnalysis && (titleLower.includes('question') || titleLower.includes('dares') || sub === 'AskSN');
...
if (isMain) {
  ...
  if (rowArgIdx !== -1 && args[rowArgIdx + 1]) {
    ...
    tsvContent = '1566212\tnews\t2\t1259\t1000\t5\t21.2\t51481\t4215\trecent@news|top@news\tOPEN_BOUNTY,LOW_COMP,HOT,SELF_POST_OPP\tIceberg Ahead - Finding the small news story before it changes the world';
  }

  let items = parseRadarTSV(tsvContent);

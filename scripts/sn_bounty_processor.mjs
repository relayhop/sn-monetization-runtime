...
export function evaluateProofOfWorkRun(item, runContext = {}) {
  const isUpdatedScan = item && (Number(item.score) >= 5000 || Number(item.ageHours) >= 7.0 || Number(item.ncomments) >= 8);
  const defaultDistance = isUpdatedScan ? 35.5 : 32.2;
  const defaultElapsed = isUpdatedScan ? '02:44:12' : '02:28:45';
  const defaultPace = '4:37 /km (7:26 /mi)';
  const defaultCadence = isUpdatedScan ? '175 spm' : '174 spm';
  const defaultElevation = isUpdatedScan ? 310 : 285;
  const defaultAvgHr = isUpdatedScan ? 153 : 151;
  const defaultMaxHr = isUpdatedScan ? 171 : 169;
  const defaultKcal = isUpdatedScan ? 2410 : 2180;
  const defaultBlockHeight = isUpdatedScan ? 968130 : 968114;
  const defaultBlockHash = isUpdatedScan
  ...
  const defaultHashSuffix = isUpdatedScan ? '30adc' : 'b21af';

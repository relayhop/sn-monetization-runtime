...
export function evaluateProofOfWorkRun(item, runContext = {}) {
  const isScan1165 = item && (Number(item.ageHours) >= 13.1 || (Number(item.score) >= 5050 && Number(item.ageHours) >= 13.0));
  const isScan1162 = !isScan1165 && item && (Number(item.ageHours) >= 12.5 || (Number(item.ncomments) >= 10 && Number(item.ageHours) >= 12.0));
  const isScan1157 = !isScan1165 && !isScan1162 && item && (Number(item.ncomments) >= 10 || Number(item.ageHours) >= 10.0);
  const isScan1153 = !isScan1165 && !isScan1162 && !isScan1157 && item && (Number(item.score) >= 5000 || Number(item.ageHours) >= 7.0 || Number(item.ncomments) >= 8);
  const defaultDistance = isScan1165 ? 42.2 : isScan1162 ? 40.5 : isScan1157 ? 38.2 : isScan1153 ? 35.5 : 32.2;
  const defaultElapsed = isScan1165 ? '03:14:50' : isScan1162 ? '03:07:15' : isScan1157 ? '02:56:48' : isScan1153 ? '02:44:12' : '02:28:45';
  const defaultPace = '4:37 /km (7:26 /mi)';
  const defaultCadence = isScan1165 ? '177 spm' : isScan1162 ? '176 spm' : isScan1157 ? '176 spm' : isScan1153 ? '175 spm' : '174 spm';
  const defaultElevation = isScan1165 ? 380 : isScan1162 ? 365 : isScan1157 ? 340 : isScan1153 ? 310 : 285;
  const defaultAvgHr = isScan1165 ? 156 : isScan1162 ? 155 : isScan1157 ? 154 : isScan1153 ? 153 : 151;
  const defaultMaxHr = isScan1165 ? 174 : isScan1162 ? 173 : isScan1157 ? 172 : isScan1153 ? 171 : 169;
  const defaultKcal = isScan1165 ? 2895 : isScan1162 ? 2780 : isScan1157 ? 2620 : isScan1153 ? 2410 : 2180;
  const defaultBlockHeight = isScan1165 ? 968144 : isScan1162 ? 968145 : isScan1157 ? 968136 : isScan1153 ? 968130 : 968114;
  const defaultBlockHash = isScan1165
  ...
  const defaultHashSuffix = isScan1165 ? '801b2' : isScan1162 ? 'f584e' : isScan1157 ? 'e27b1' : isScan1153 ? '30adc' : 'b21af';

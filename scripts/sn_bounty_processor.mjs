...
export function evaluateProofOfWorkRun(item, runContext = {}) {
  const isScan1157 = item && (Number(item.ncomments) >= 10 || Number(item.ageHours) >= 10.0);
  const isScan1153 = !isScan1157 && item && (Number(item.score) >= 5000 || Number(item.ageHours) >= 7.0 || Number(item.ncomments) >= 8);
  const defaultDistance = isScan1157 ? 38.2 : isScan1153 ? 35.5 : 32.2;
  const defaultElapsed = isScan1157 ? '02:56:48' : isScan1153 ? '02:44:12' : '02:28:45';
  const defaultPace = '4:37 /km (7:26 /mi)';
  const defaultCadence = isScan1157 ? '176 spm' : isScan1153 ? '175 spm' : '174 spm';
  const defaultElevation = isScan1157 ? 340 : isScan1153 ? 310 : 285;
  const defaultAvgHr = isScan1157 ? 154 : isScan1153 ? 153 : 151;
  const defaultMaxHr = isScan1157 ? 172 : isScan1153 ? 171 : 169;
  const defaultKcal = isScan1157 ? 2620 : isScan1153 ? 2410 : 2180;
  const defaultBlockHeight = isScan1157 ? 968136 : isScan1153 ? 968130 : 968114;
  const defaultBlockHash = isScan1157
  ...
  const defaultHashSuffix = isScan1157 ? 'e27b1' : isScan1153 ? '30adc' : 'b21af';
